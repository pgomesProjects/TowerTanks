...
public class TextPulse : MonoBehaviour
{
    ...
    {
        if (loop)
            LeanTween.alphaCanvas(GetComponent<CanvasGroup>(), endAlpha, seconds).setLoopPingPong();
        else
            LeanTween.alphaCanvas(GetComponent<CanvasGroup>(), endAlpha, seconds).setLoopPingPong(1);
    }
}
