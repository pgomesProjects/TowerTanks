...
namespace TowerTanks.Scripts
{
    ...
    public class Room : MonoBehaviour
    {
        ...
        private Color defaultRoomColor;                              //The default color for the cells in the room
        private Color invalidPlacementColor = Color.red;             //The color for a room that cannot be placed
        ...
        [Tooltip("Whether or not this room meets the criteria to be attached to a room.")]                         internal bool canBeMounted = false;
        ...
        [Tooltip("Whether or not the room is obstructed by any objects while building.")]                          internal bool isObstructed = false;
        ...
        public void Initialize()
        {
            ...
            if (backWallSprite != null)
                defaultRoomColor = backWallSprite.color;
            else
            {
                if (cells[0].backWall.TryGetComponent(out SpriteRenderer backWallRenderer))
                    defaultRoomColor = backWallRenderer.color;
            }
        }
        ...
        public Vector2 SnapMove(Vector2 targetPoint)
        {
            ...
            //If there are no connections, change the color to red
            canBeMounted = ghostCouplers.Count != 0;
            if (!canBeMounted)
                ChangeRoomColor(invalidPlacementColor);
            else
                ChangeRoomColor(defaultRoomColor);
        ...
        public bool Mount()
        {
            ...
            if (!canBeMounted) { Debug.Log("Tried to mount room which is not connected to any other rooms!"); return false; } //Cannot mount rooms which are not connected to the tank
            ...
            ChangeRoomColor(defaultRoomColor);                     //Changes the room color to the default room color
            return mounted;
        }
        ...
        public void UpdateRoomType(RoomType newType)
        {
            ...
            //Change room color:
            ChangeRoomColor(roomData.roomTypeColors[(int)newType]);
        }

        /// <summary>
        /// Refreshes the collision of the room.
        /// </summary>
        /// <param name="collisionEnabled">If true, collision is enabled for the room. If false, collision is disabled.</param>
        private void RefreshCollision(bool collisionEnabled)
        {
            //If no value has changed, return
            if (isObstructed != collisionEnabled)
                return;
            
            isObstructed = !collisionEnabled;
            ChangeRoomColor(collisionEnabled ? defaultRoomColor : invalidPlacementColor);
            EnableRoomCollision(collisionEnabled);
        }
        ...
        /// <summary>
        /// Enables or disables collision for a room.
        /// </summary>
        /// <param name="isEnabled">If true, collision is enabled. If false, collision is disabled.</param>
        private void EnableRoomCollision(bool isEnabled)
        {
            foreach(Cell cell in cells) //Iterate through each cell in room
            {
                //Get all of the colliders in the cell
